...
> * SQL
<pre>select date, m_type, avg(volume) as avg_volume
  from trading_stock_marketdata
 where date > '2021-00-00' 
 group by date, m_type
having sum(volume) > 2000000000; </pre>

> * Django ORM
<pre>StockMarketData.objects
    .filter(date > '2021-00-00')                                    #1
    .values('date', 'm_type')                                       #2
    .annotate(sum_volume=Sum('volume'), avg_volume=Avg('volume'))   #3
    .filter(sum_volume__gt=2000000000)                              #4
    .values('avg_volume')</pre>
